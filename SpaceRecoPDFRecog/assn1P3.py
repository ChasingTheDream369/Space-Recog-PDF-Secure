...
def MakeLine(ls,b):
    ...
    l= ls[0]
    # INV5(b1,g1): b1 >= g1 and b = b1 + "the number of avaialble spaces already 
    # mapped between the words"

    for w in ls[1:]:
        # Number of blanks to put in this gap.
        r = (b1/g1) if (b1%g1 == 0) else floor(b1/g1) 
        
        assert floor(b/g)<=r<=ceil(b/g), "Space must not vary too much."
        l = l + ' '*(int(r)) + w 
        b1,g1= b1-r, g1 -1

    # b1==0 here, because all the available spaces have 
    # been mapped between words correctly
    # maintaining the invariant correctly, it sorts of becomes 
    # the post condition for maintaining the above criteria and when the 
    # condition gets falsified for the loop then at that point following 
    # from the hoare triple of loops the Invariant and the !loop condtion exists,
    # for the invariant to exists i.e b1 >= g1 the value of b1 has to be 
    # equal to 0 as g is 0 at the end of the loop and just before the last
    # condtion always the value of b1 and g1 gets ewual and as the loop body has
    # as decreasing varaint therefore when the condtion gets falsified 
    # both of them have to be equal and b1 cant be greater than g1 and therfore 
    # b1 will be 0(also at the end of condtion the value of M gets zero 
    # which inherently states that the value of b1 should be 0)
    return l
